...
export function FileUpload({ onBack }: FileUploadProps) {
  ...
  const validateFile = (f: File): boolean => {
    ...
    if (!/\.(mp3|wav|m4a|ogg|flac|mp4)$/i.test(f.name)) {
  ...
  const handleProcess = async () => {
    ...
    // Progreso animado mientras espera respuesta del backend
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err?.detail?.message || 'Error al procesar el audio')
      }
    ...
    <div>
      ...
      <div className="bg-white/[0.04] backdrop-blur-sm border border-white/8 rounded-2xl p-7">
        ...
        <p className="text-white/35 text-sm mb-6">
          Sube un archivo pregrabado — recibirás transcripción completa + informe con puntos clave, tareas y decisiones
        </p>
          ...
          <AnimatePresence mode="wait">
            {!file ? (
              ...
              >
                ...
                <p className="text-white/20 text-xs mt-4">MP3, WAV, M4A — máx. {MAX_SIZE_MB}MB</p>
              </motion.div>
            ) : (
              <motion.div
                key="file"
                  ...
                  {uploadState === 'uploading' && (
                    <div className="mt-2">
                      ...
                      <p className="text-white/30 text-[11px] mt-1">
                        {progress < 30 ? 'Transcribiendo audio...' : progress < 60 ? 'Generando resumen...' : progress < 85 ? 'Extrayendo tareas...' : 'Finalizando...'}
                      </p>
                    </div>
                  )}
                </div>
        ...
        {uploadState === 'uploading' && (
          <p className="text-center text-white/25 text-xs mt-3">
            La primera vez puede tardar 2-5 min mientras se cargan los modelos de IA
          </p>
        )}
      </div>
    </div>
