...
Cubre:
  ...
  - _fallback: análisis directo sin gRPC via FlanAdapter

Ejecutar:
    cd backend
    python -m pytest tests/unit/test_grpc_client.py -v
"""
...
class TestFallback:
    def test_fallback_directo(self):
        ...
        with patch.object(FlanAdapter, "analyze") as mock_analyze:
            mock_analyze.return_value = AnalysisResult(
                summary="Resumen directo",
                key_points=["Punto A"],
                tasks=["Tarea B"],
                decisions=["Decisión C"],
                via_grpc=False,
            )
